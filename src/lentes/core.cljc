...
(defn derive
  ...
  ([lens src {:keys [read-only? check-equals?]
   ...
   (let [id (str (gensym "lentes"))
         ...
         srccache (volatile! +empty+)
         oldcache (volatile! +empty+)
         oldsrccache (volatile! source)]
     (if read-only?
